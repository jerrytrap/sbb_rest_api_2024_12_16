...
export default function UserInfo() {
  ...
  return (
    <div className="container mx-auto px-4">
      ...
      <h5 className="my-3 border-b pb-2 text-lg font-semibold">
        내가 질문한 글
      </h5>
        ...
        <tbody>
          {answers.map((answer) => (
            <tr key={answer.id} className="text-center">
              <td className="px-4 py-2">
                <a
                  href={`/questions/detail/${answer.questionId}`}
                  className="text-blue-500 hover:underline"
                >
                  {answer.questionId}
                </a>
        ...
        <tbody>
          {comments.map((comment) => (
            <tr key={comment.id} className="text-center">
              <td className="px-4 py-2">
                <a
                  href={`/questions/detail/${comment.questionId}`}
                  className="text-blue-500 hover:underline"
                >
                  {comment.questionId}
                </a>
